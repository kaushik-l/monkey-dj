...
%             data = data(data<prctile(data,99));
            ...
            indx = randsample(numel(data),min(numel(data),200));
            if length(density) > 1
                jitterstrength = interp1(value, density*width, data(indx));
            else % all data is identical:
                jitterstrength = density*width;
            end
            jitter = 2*(rand(size(data(indx)))-0.5);
            obj.ScatterPlot = ...
                scatter(pos + jitter.*jitterstrength, data(indx), 0.5);
    ...
    methods (Access=private)
        function results = checkInputs(obj, data, pos, varargin)
            ...
            p.addParameter('BoxColor', [1 1 1], iscolor);
